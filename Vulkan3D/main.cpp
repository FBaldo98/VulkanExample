...
////	PIPELINE CREATION
	...
	void createImageViews() {
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			// SubresourceRange describes what the image's purpose is and which part of the image to access
	...
	void createLogicalDevice() {
		...
		for (uint32_t queueFamilu : uniqueQueueFamilies) {
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		// Search the graphics family index
		int i = 0;
	...
	void pickPhysicalDevice() {
		...
		// Check if device is suitable for application
	...
	void checkExtensions() {
		...
		for (const char* extension : getRequiredExtensions()) {
	...
	void drawFrame() {
		...
		// Acquire the image from swap chain
		uint32_t imageIndex;
		...
		// Check if previous frame is usint this image
		if (imagesInFlight[imageIndex] != VK_NULL_HANDLE) {
			vkWaitForFences(device, 1, &imagesInFlight[imageIndex], VK_TRUE, UINT64_MAX);
		}
		// Mark the image as now being in use
